...
try:
    # Streamlit Cloud 배포 시 st.secrets에서 키를 가져옴
    client = OpenAI(api_key=st.secrets["openai"]["api_key"])
except KeyError:
    # 로컬 환경 변수에서 키를 가져옴 (개발 환경)
...
def initialize_session(role_name, prompt):
    """역할이 변경될 경우 대화 기록을 초기화합니다."""
    # 시스템 메시지를 대화의 첫 번째 요소로 설정
    st.session_state["messages"] = [{"role": "system", "content": prompt}]
    st.session_state["current_role"] = role_name

# 역할이 바뀌었는지 확인하고, 바뀌었으면 초기화
...
# 5. 대화 기록 표시
# 시스템 메시지를 제외하고 사용자(user)와 AI(assistant) 메시지만 표시
    ...
    with st.chat_message("assistant"):
        with st.spinner(f"**{selected_role_name}**가 생각 중..."):
            try:
                ...
                st.error(f"오류 발생: {e}")
                # 오류 발생 시 사용자 메시지 제거하여 재시도 가능하도록 함
