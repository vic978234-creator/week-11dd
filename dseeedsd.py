...
# 🚨 수정된 부분: "OpenA"가 아닌 "OpenAI"로 정확히 import 합니다.
from openai import OpenAI
import os

# 1. OpenAI 클라이언트 초기화 및 API 키 설정
try:
    # 🌟 Streamlit Cloud 또는 secrets.toml에서 API 키 불러오기
    # API 키를 코드에 직접 노출하지 않고 안전하게 사용합니다.
    client = OpenAI(api_key=st.secrets["openai"]["api_key"])
except KeyError:
    # 🌟 로컬 개발 환경에서 환경 변수에서 키 불러오기
    client = OpenAI(api_key=os.environ.get("OPENAI_API_KEY"))
    if not client.api_key:
        st.error("🚨 OpenAI API 키를 설정해주세요 (.streamlit/secrets.toml 또는 환경 변수).")
        st.stop()
